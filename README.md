...
- **결정론 우선**: 계약방법 결정은 LLM이 아닌 룰엔진이 수행 — 같은 입력엔 항상 같은
  결과. LLM은 위저드의 계약유형 분류·설명 생성에만 사용(키 없이도 핵심 기능 동작).
  웹 법령 챗봇(`/api/v1/ask`)은 2026-08-15자로 폐지 — 법령 질의는 MCP 축이 담당한다
...
tests/      단위·회귀 테스트 + 질문은행(question_bank.json)
```
...
법령 근거 검색(MCP `search_law`·`search_references`)을 쓰려면 공개 코퍼스를 인덱싱합니다:
