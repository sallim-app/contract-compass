...
4. **수리한 결함은 `tools/mcp_regression.py`에 `R*` 회귀로 박는다.** 기대값은 반드시 실측으로
   확정(지어낸 문항은 회귀를 헛돌게 한다). 사람이 읽는 문항지는 루트 `evaluation.xml`.
5. **도구·응답 필드가 바뀌면 `SERVER_VERSION` 마이너 범프 + `server.json` 동기.** 클라이언트는
   옛 도구 목록·응답 계약을 캐시하므로 `/mcp/health`의 version이 스테일 판별의 유일한 단서다.
6. **이 파일은 얇게 유지한다** — 상세는 `docs/`. 새 함정은 여기 쌓지 말고 해당 문서에 넣어라.
